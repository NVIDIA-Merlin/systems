...
def test_example_04_exporting_ranking_models(tb, tmpdir):
    ...
    workflow.save(str(tmpdir / "workflow"))
    ...
    model.save(str(tmpdir / "dlrm"))
    tb.inject(
        f"""
        import os
        os.environ["INPUT_FOLDER"] = "{str(tmpdir)}"
        """
    )
    ...
    batch = df_lib.read_parquet(
        os.path.join(tmpdir, "valid", "part.0.parquet"),
        num_rows=3,
    ...
    response = run_ensemble_on_tritonserver(
        f"{str(tmpdir)}/ensemble/", schema.without(["click"]), batch, outputs, "executor_model"
    )
