...
def test_feast_transform(prefix, is_ragged):
    ...
    FeatureStore.__init__ = MagicMock(return_value=None)
    FeatureStore.get_online_features = MagicMock(return_value=mocked_resp)
...
def test_feast_config_round_trip():
    ...
    This is a bad look, but this test needs to be last in the file because we mock
    QueryFeast.__init__, which somehow globally modifies that class. So other tests that call
    the QueryFeast constructor must do so before this test.
    """
    input_schema = Schema()
    output_schema = Schema()

    # Need to mock the FeatureStore constructor so it doesn't look for a real store
    FeatureStore.__init__ = MagicMock(return_value=None)
    ...
    QueryFeast.__init__ = MagicMock(return_value=None)
    QueryFeast.from_config(created_config_dict)
    assert QueryFeast.__init__.called_with(*args, **kwargs)
